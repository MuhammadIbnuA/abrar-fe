  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        <input type="text" value={nama} onChange={(e) => setNama(e.target.value)} placeholder="Nama Benda" />
        <input type="text" value={harga} onChange={(e) => setHarga(e.target.value)} placeholder="Kategori" />
        <input type="text" value={kategori} onChange={(e) => setKategori(e.target.value)} placeholder="Jumlah" />
        <button type="submit">{selectedMenuId ? "Update Data" : "Create Data"}</button>
      </form>

      <h1>DAFTAR DATA INVENTARIS KANTOR</h1>
      <table>
        <thead>
          <tr>
            <th>Nama Benda</th>
...
async function createMenu(menuData) {
  try {
    ...
    const response = await fetch("https://abrar-be.vercel.app/menu", {
      method: "POST",
      headers: {
